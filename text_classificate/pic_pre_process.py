...
def auto_detect_page_corners(img):
    ...
    adjusted_points = np.array([
        [tl[0], top_point[1]],    # 左上保持X，Y设为全局最高
        [tr[0], top_point[1]],    # 右上保持X，Y设为全局最高 
        [br[0], bottom_point[1]], # 右下保持X，Y设为全局最低
        [bl[0], bottom_point[1]]  # 左下保持X，Y设为全局最低
    ], dtype=np.float32)
